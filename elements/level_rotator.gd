...
static func next_level(from_child: Node):
	var level_rot = _find_level_rotator(from_child)
	if level_rot != null:
		# Jingle does not apply to level 0 which is the lore dump
		...
		if level_rot.current_level >= len(all_level_files):
			...
			level_rot._start_level.call_deferred(level_rot.current_level)
